...
export class BookDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.userMailId = "Satheesh@gmail.com";
    this.userRole = "Admin";
    this.userName = "Satheesh S";
    ...
    this.bookReviewService.getAllRatings()
      .subscribe(
        (response) => {
          ...
          for (const id in response) {
            if (response.hasOwnProperty(id)) {
              if (response[id].Isbn === this.fireBook.Isbn) {
                this.userRatings.push(response[id]);
              }
            }
          }
        }
